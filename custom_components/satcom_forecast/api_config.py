...
class ConfigManager:
    ...
    def _load_from_environment(self):
        ...
        self.api_config.base_url = os.getenv(
            "WEATHER_API_BASE_URL", self.api_config.base_url
        )
        self.api_config.user_agent = os.getenv(
            "WEATHER_API_USER_AGENT", self.api_config.user_agent
        )
        self.api_config.timeout = int(
            os.getenv("WEATHER_API_TIMEOUT", str(self.api_config.timeout))
        )
        self.api_config.retry_attempts = int(
            os.getenv("WEATHER_API_RETRY_ATTEMPTS", str(self.api_config.retry_attempts))
        )
        self.api_config.retry_delay = float(
            os.getenv("WEATHER_API_RETRY_DELAY", str(self.api_config.retry_delay))
        )
        self.api_config.rate_limit_delay = float(
            os.getenv(
                "WEATHER_API_RATE_LIMIT_DELAY", str(self.api_config.rate_limit_delay)
            )
        )
        self.api_config.enable_alerts = (
            os.getenv("WEATHER_API_ENABLE_ALERTS", "false").lower() == "true"
        )
        self.api_config.cache_duration = int(
            os.getenv("WEATHER_API_CACHE_DURATION", str(self.api_config.cache_duration))
        )
        self.api_config.max_cache_size = int(
            os.getenv("WEATHER_API_MAX_CACHE_SIZE", str(self.api_config.max_cache_size))
        )

        # Feature Flags
        self.feature_flags.use_api = (
            os.getenv("WEATHER_USE_API", "true").lower() == "true"
        )
        self.feature_flags.fallback_to_html = (
            os.getenv("WEATHER_FALLBACK_HTML", "true").lower() == "true"
        )
        self.feature_flags.enable_alerts = (
            os.getenv("WEATHER_ENABLE_ALERTS", "false").lower() == "true"
        )
        self.feature_flags.enable_caching = (
            os.getenv("WEATHER_ENABLE_CACHING", "true").lower() == "true"
        )
        self.feature_flags.enable_metrics = (
            os.getenv("WEATHER_ENABLE_METRICS", "true").lower() == "true"
        )
        self.feature_flags.debug_mode = (
            os.getenv("WEATHER_DEBUG_MODE", "false").lower() == "true"
        )
