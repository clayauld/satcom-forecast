...
class CacheManager:
    ...
    def get_cache(self, cache_name: str, max_size: int = 1000, default_ttl: int = 300) -> APICache:
        """
        ...
        if cache_name not in self._caches:
            self._caches[cache_name] = APICache(max_size=max_size, default_ttl=default_ttl)
        ...
        async def cleanup_loop():
            while True:
                try:
                    ...
                    if total_cleaned > 0:
                        _LOGGER.debug(f"Background cleanup removed {total_cleaned} expired entries")
