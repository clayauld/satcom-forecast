...
from .api_cache import get_forecast_cache, get_gridpoint_cache
...
class ForecastFetcherAPI:
    ...
    async def fetch_forecast_api(
        ...
        try:
            ...
            # Get forecast data
            forecast_data = await self._get_forecast(
                office, grid_x, grid_y, forecast_url
            )
    ...
    async def fetch_forecast_hybrid(
        self, lat: float, lon: float, days: Optional[int] = None, mode: str = "auto"
    ) -> str:
        """
    ...
    async def _get_forecast(
        self, office: str, grid_x: int, grid_y: int, forecast_url: Optional[str] = None
    ) -> Dict[str, Any]:
        """
    ...
    def _convert_to_text_format(
        self, forecast_data: ForecastData, days: Optional[int] = None
    ) -> str:
        """
        ...
        if days is not None:
            ...
            periods_to_include = weather_utils.filter_periods_by_days(
                forecast_data.periods, days
            )
...
async def fetch_forecast_hybrid(
    lat: float, lon: float, days: Optional[int] = None, mode: str = "auto"
) -> str:
...
async def fetch_forecast(lat: float, lon: float, days: Optional[int] = None) -> str:
    ...
    try:
        ...
        lat_float = float(lat)
        lon_float = float(lon)
        return await fetcher.fetch_forecast_hybrid(
            lat_float, lon_float, days, mode="auto"
        )
    except Exception as e:
