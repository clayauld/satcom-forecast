...
class WeatherGovAPIClient:
    ...
    async def _make_request(self, url: str, method: str = "GET") -> APIResponse:
        ...
        for attempt in range(self.retry_attempts):
            try:
                ...
                async with self._session.request(method, url) as response:
                    ...
                    _LOGGER.debug(
                        f"Response status: {response.status} in {response_time:.2f}s"
                    )
        ...
        response_time = loop.time() - start_time
        return APIResponse(
            success=False, error="Max retries exceeded", response_time=response_time
        )
    ...
    async def get_gridpoint(
        ...
        try:
            ...
            _LOGGER.debug(
                f"Grid point: office={office}, grid_x={grid_x}, grid_y={grid_y}"
            )
    ...
    async def get_forecast(
        ...
        _LOGGER.debug(
            f"Fetching forecast for {office}/{grid_x},{grid_y} using URL: {url}"
        )
        ...
        try:
            ...
            _LOGGER.debug(
                f"Retrieved forecast with {len(properties['periods'])} periods"
            )
            return data
