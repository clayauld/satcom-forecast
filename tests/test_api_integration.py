...
class TestAPIIntegration:
    ...
    async def test_get_forecast_real_api(self, client):
        ...
        print(f"Retrieved forecast with {len(forecast_data['properties']['periods'])} periods")
    ...
    async def test_forecast_fetcher_api_mode(self, fetcher):
        ...
        # Enable API mode
        with patch('custom_components.satcom_forecast.api_config.is_api_enabled', return_value=True):
    ...
    async def test_forecast_fetcher_hybrid_mode(self, fetcher):
        ...
        with patch('custom_components.satcom_forecast.api_config.is_api_enabled', return_value=True):
            forecast_text = await fetcher.fetch_forecast_hybrid(40.7128, -74.0060, 2, mode='auto')
        ...
        with patch('custom_components.satcom_forecast.api_config.is_api_enabled', return_value=False):
            forecast_text = await fetcher.fetch_forecast_hybrid(40.7128, -74.0060, 2, mode='auto')
