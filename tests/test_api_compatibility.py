...
from custom_components.satcom_forecast.forecast_fetcher_api import (
    fetch_forecast as fetch_forecast_api,
)
    ...
    async def test_forecast_text_structure(self, test_coordinates):
        ...
        for lat, lon in test_coordinates:
            try:
                ...
                assert any(
                    period in html_forecast
                    for period in ["Today:", "Tonight:", "Monday:", "Tuesday:"]
                )
                assert any(
                    period in api_forecast
                    for period in ["Today:", "Tonight:", "Monday:", "Tuesday:"]
                )
                ...
                print(
                    f"Length difference: {abs(len(html_forecast) - len(api_forecast))}"
                )
    ...
    async def test_format_compatibility_summary(self, test_coordinates):
        ...
        for lat, lon in test_coordinates:
            try:
                ...
                # Both should contain period indicators
                assert any(
                    period in html_summary
                    for period in ["Tdy:", "Tngt:", "Mon:", "Tue:"]
                )
                assert any(
                    period in api_summary
                    for period in ["Tdy:", "Tngt:", "Mon:", "Tue:"]
                )

                print(
                    f"HTML summary: {len(html_summary)} chars - {html_summary[:100]}..."
                )
    ...
    async def test_format_compatibility_compact(self, test_coordinates):
        ...
        for lat, lon in test_coordinates:
            try:
                ...
                # Both should contain period indicators
                assert any(
                    period in html_compact
                    for period in ["Today:", "Tonight:", "Monday:", "Tuesday:"]
                )
                assert any(
                    period in api_compact
                    for period in ["Today:", "Tonight:", "Monday:", "Tuesday:"]
                )
    ...
    async def test_wind_extraction_compatibility(self):
        ...
        for period in test_periods:
            ...
            if period.wind_speed and period.wind_direction:
                ...
                assert (
                    period.wind_direction[:2] in wind_info
                )  # Should contain direction abbreviation
    ...
    async def test_character_count_targets(self, test_coordinates):
        ...
        for lat, lon in test_coordinates:
            try:
                ...
                assert (
                    80 <= len(summary) <= 150
                ), f"Summary length {len(summary)} not in target range 80-150"
                assert (
                    400 <= len(compact) <= 1500
                ), f"Compact length {len(compact)} not in target range 400-1500"
    ...
    async def test_content_quality_comparison(self, test_coordinates):
        ...
        for lat, lon in test_coordinates:
            try:
                ...
                html_terms = sum(
                    1 for term in weather_terms if term.lower() in html_forecast.lower()
                )
                api_terms = sum(
                    1 for term in weather_terms if term.lower() in api_forecast.lower()
                )
...
class TestRegressionCompatibility:
    ...
    async def test_existing_imports_still_work(self):
        ...
        from custom_components.satcom_forecast.api_data_processor import (
            APIDataProcessor,
        )
        from custom_components.satcom_forecast.api_formatter import APIFormatter

        # Test that coordinator can still import everything
        from custom_components.satcom_forecast.coordinator import (
            SatcomForecastCoordinator,
        )
        from custom_components.satcom_forecast.forecast_fetcher import (
            fetch_forecast as original_fetch,
        )
    ...
    async def test_backward_compatibility(self):
        ...
        from custom_components.satcom_forecast.forecast_fetcher import (
            fetch_forecast as old_fetch,
        )
        from custom_components.satcom_forecast.forecast_fetcher_api import (
            fetch_forecast as new_fetch,
        )
