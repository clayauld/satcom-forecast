...
async def test_step_user_success(mock_hass):
    ...
    with patch.object(flow, 'async_step_finalize', new_callable=AsyncMock) as mock_finalize:
...
async def test_step_user_connection_error(mock_hass):
    ...
    assert result["type"] == FlowResultType.FORM
    assert result["errors"] == {"base": "imap_invalid", "imap_host": "Connection refused"}
...
async def test_step_finalize_invalid_folder(mock_hass):
    ...
    assert result["type"] == FlowResultType.FORM
    assert result["errors"] == {"base": "invalid_folder", "imap_folder": "Folder not found"}

def test_get_imap_folders():
    """Test get_imap_folders function."""
    with patch("imaplib.IMAP4_SSL") as mock_imap:
        mock_instance = mock_imap.return_value
        mock_instance.list.return_value = ("OK", [b'(\\HasNoChildren) "/" "INBOX"', b'(\\HasNoChildren) "/" "Sent"'])
...
def test_validate_imap_folder():
    ...
    with patch("imaplib.IMAP4_SSL") as mock_imap:
        ...
        valid, error = validate_imap_folder("host", 993, "user", "pass", "Invalid", "SSL")
...
async def test_options_flow_update(mock_hass, mock_config_entry):
    ...
    with patch("custom_components.satcom_forecast.async_reload_entry", side_effect=mock_reload) as mock_reload_mock:
