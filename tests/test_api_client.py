...
class TestWeatherGovAPIClient:
    ...
    async def test_validate_response_success(self, client):
        ...
        response = APIResponse(
            success=True, data={"properties": {"test": "data"}}, status_code=200
        )
    ...
    async def test_validate_response_failure(self, client):
        ...
        with pytest.raises(
            APIError, match="Missing 'properties' or 'features' in API response"
        ):
            client._validate_response(response)
    ...
    async def test_get_gridpoint_success(self, client):
        ...
        mock_response = APIResponse(
            success=True,
            data={"properties": {"cwa": "OKX", "gridX": 32, "gridY": 34}},
            status_code=200,
        )
    ...
    async def test_get_gridpoint_missing_field(self, client):
        ...
        with patch.object(client, "_make_request", return_value=mock_response):
            with pytest.raises(
                APIError, match="Missing required field in grid point response"
            ):
    ...
    async def test_get_forecast_missing_periods(self, client):
        ...
        mock_response = APIResponse(
            success=True, data={"properties": {"invalid": "data"}}, status_code=200
        )

        with patch.object(client, "_make_request", return_value=mock_response):
            with pytest.raises(
                APIError, match="Missing 'periods' in forecast response"
            ):
...
class TestAPIResponse:
    ...
    def test_api_response_creation(self):
        ...
        response = APIResponse(
            success=True, data={"test": "data"}, status_code=200, response_time=1.5
        )
    ...
    def test_api_response_with_error(self):
        ...
        response = APIResponse(success=False, error="Test error", status_code=404)
